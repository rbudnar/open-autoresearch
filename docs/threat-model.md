...
| A record's `source_commit` doesn't resolve / reconstruct the experiment's true code state | Provenance is content-addressed Level 1: the commit is a **non-authoritative breadcrumb** (campaigns run off-main; squash-merge destroys SHAs), not a reproducibility guarantee. The schema requires a provenance shape (triple or legacy pair) but never resolves commits. | `resolvable_from_main` makes unauditability VISIBLE. True code/data/env reproduction is Level 2/3 (run-start capture + content-addressed store + `reproduce`), tracked in `docs/proposals/2026-06-13-provenance-redesign.md` + ADR 0001. |
| Under `mode: declarative` (§6.3.1) the agent re-materializes the split — the held-out membership is regenerable, so the frozen "agent-can't-regenerate" immutability no longer holds | Declarative mode is mechanism + guidance, not a mandate: it trades immutability for the ability to track a growing dataset. An agent with the rule, seed, and dataset can reproduce (or quietly shift) the split. | **Frozen stays the recommended default**; §6.3.1 recommends frozen or a **non-agent materialization** (CI/human freezes the declarative output) for deployment-grade. The recorded `data_fingerprint` split identity + the verifier's `cross_dataset` warning (rule 11) keep any divergence between a baseline and candidate VISIBLE on the packet rather than silent. Dataset growth that changes the holdout is a holdout refresh (`val_set_version` bump, §17.6.3). |
