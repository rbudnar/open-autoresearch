...
2. **Per-node content and state** — node `title`/`lessons` come from the records' optional curated fields `node_title` / `node_lessons` (falling back to the record's `hypothesis` / `lessons` when those curated fields are absent). Optional `lifecycle_status`, `promotion_status`, `frontier_eligible`, `blocked_by`, `pruned_reason`, `merged_into`, and `node_type` fields feed derived operational views.
